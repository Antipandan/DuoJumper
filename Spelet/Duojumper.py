...
def reset_pos(player_group: pygame.sprite.Group):
    for player in player_group.sprites():
        """
                level.player_1.x_pos = start_pos[0]
        level.player_1.y_pos = start_pos[1]
        """
...
while RUNNING:
    ...
    if in_main:
        ...
        if level_select:
            screen.fill("black")
            render_level_only(screen, level_path + "Showcase_levels/" + chosen_map_path)
            map_select = button_centered("Select a map", screen_width / 2, 10, 60)
            map_name_1 = images_path + "map_img_2.jpg"
            image_level_1 = RenderLevelIcon(58, 90, map_name_1, "Wanderers Rest", level_path + "level_1/Test.tmx")
            ...
            image_level_2 = RenderLevelIcon(148 + 210, 90, images_path + "map_img_4.jpg", "Floating point",
                                            level_path + "level_1/Level_2.tmx")
            ...
            image_level_3 = RenderLevelIcon(148 + (90 * 3) + 210, 90, images_path + "map_img_5.jpg", "Hip Hop Hill",
                                            level_path + "level_1/Level_3.tmx")
            maps = [image_level_1, image_level_2, image_level_3]
            main_rect = back_to_main_menu(0, 0, 45)
            # mapa för jag är trött just nu :)>
            for mapa in maps:
                if mapa.render_level_icon().collidepoint(mouse_pos) and mouse_input[0] and mapa.map_location != "":
                    level_select = False
                    in_game = True
                    in_main = False
                    paused = False
                    level = Level(screen, mapa.map_location)
            pygame.display.update()
            ...
            if not all_alive:
                ...
                # mouse_rect = pygame.Rect(mouse_pos, (20, 20))
                # pygame.draw.rect(screen, (255, 0, 0, 120), mouse_rect , 1)
                pygame.display.update()
